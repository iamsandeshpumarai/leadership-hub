...
const AboutAuthor = ({ authorData  }) => {
  ...
  const authorFeatures = authorData?.data?.authortags?.map((tag) => {
    ...
    if (tag.badge === 'achievement') {
      ...
      icon = null; // Fallback if enum expands
    }
    return { icon, title: tag.role }; // Use role as title
  });
  ...
  return (
    <section className="mt-12 mb-12 bg-red-900 p-8 lg:p-12 transform transition-all duration-1000 delay-500 translate-y-0 opacity-100">
      <div className="max-w-4xl mx-auto text-center">
        ...
        {/* Author Description - From data */}
        <p className="text-red-100 leading-relaxed text-lg mb-8">
          {authorData?.data?.aboutauthor || 'No author description available.'} {/* Fallback */}
        </p>

        {/* Author Features Grid - Dynamic from authortags */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-center">
          ...
          {authorFeatures?.length === 0 && <p className="text-red-200">No author tags available.</p>}
        </div>
      </div>
    </section>
  );
